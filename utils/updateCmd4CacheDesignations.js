...
// Copy the Cmd4 Designations we do not want cached, to the cached accessory in homeBridge.
function updateCmd4CacheDesignations( api, log, existingAccessory, accessory, existingAccessorysArray, existingAccessorysArrayIndex )
{
   ...
   // These are the Cmd4 Designations we want to use from the accessories config file
   // The != condition is the case where both are undefinee, poluting the targer
   if ( existingAccessory.polling != accessory.polling )
      existingAccessory.polling = accessory.polling;
   if ( existingAccessory.state_cmd != accessory.state_cmd )
      existingAccessory.state_cmd = accessory.state_cmd;
   if ( existingAccessory.state_cmd_prefix != accessory.state_cmd_prefix )
      existingAccessory.state_cmd_prefix = accessory.state_cmd_prefix;
   if ( existingAccessory.state_cmd_suffix != accessory.state_cmd_suffix )
      existingAccessory.state_cmd_suffix = accessory.state_cmd_suffix;
   if ( existingAccessory.outputConstants != accessory.outputConstants )
      existingAccessory.outputConstants = accessory.outputConstants;
   if ( existingAccessory.stateChangeResponseTime != accessory.stateChangeResponseTime )
      existingAccessory.stateChangeResponseTime = accessory.stateChangeResponseTime;
   if ( existingAccessory.fakegato != accessory.fakegato )
      existingAccessory.fakegato = accessory.fakegato;
   if ( existingAccessory.UUID != accessory.UUID )
      existingAccessory.UUID = accessory.UUID;
   if ( existingAccessory.fetch != accessory.fetch )
      existingAccessory.fetch = accessory.fetch;
   if ( existingAccessory.allowTLV8 != accessory.allowTLV8 )
      existingAccessory.allowTLV8 = accessory.allowTLV8;
   if ( existingAccessory.category != accessory.category )
   ...
   // This routine is kind of recursive, in that it handles the linked types and added accessories
   // If that is the case, place the modded existing accessory Back into its array and feturn;
   ...
   if ( Array.isArray( accessory.linkedTypes ) )
   {
      ...
      if ( existingAccessory.linkedAccessoriesConfig )
      {
         ...
         for ( let index=0; index < accessory.linkedTypes.length; index++ )
         {
            ...
            // See if the linked accessory config exists in the existing accessories linked types array.
            // This time get its index into the array, so we can put it back, modded.
            ...
            if ( existingLinkedAccessorysArrayIndex != -1 )
            {
               ...
               // Call this same mod function, this time passing the array of where it came from and 
               // it's index, so that omce modded, it will be replaced in the array at the same index
               // it came from.
               updateCmd4CacheDesignations( api, log,
               ...
               // There is a new linked accessory that is not in the existing array of linked accessories
         ...
         // It is possable that there are now Linked accessories where there
         // was none before. When there are none, then they are all the new ones.
   ...
   if ( Array.isArray( accessory.accessories ) )
   {
      ...
      if ( existingAccessory.accessoriesConfig )
      {
         ...
         {
            // First get the added accessorys config and make sure it has a name.
            ...
            // Use the UUID to find the accessory in the array of accessoriesConfig
            // in the same manner we find the existing accessory.
            addedAccessorysConfig.UUID = addedAccessorysConfig.UUID || getAccessoryUUID( addedAccessorysConfig, api.hap.uuid );

            // See if the added accessory config exists in the existing accessories added accessories array.
            // This time get its index into the array, so we can put it back, modded.
            ...
            if ( existingAddedAccessorysArrayIndex != -1 )
            {
               ...
               // There is a new added accessory that is not in the existing array of added accessories
   ...
   //
   // The existingAccessory has all been updated, accept in the case where the existing
   // linkedTypes or Added Accessories have an entry to be removed.
   //
   // log.debug( "\n\n*** linkedAccessories Deletions ***" );

   // So now go the other way and look for the linkedAccessories that must be removed.
   if ( Array.isArray( existingAccessory.linkedAccessoriesConfig ) )
   {
      ...
      if ( Array.isArray( accessory.linkedTypes ) )
      {
         // Iterate over each of the existing configurations of the linked accessories, backwards
         // so it is easy to remove them.
         ...
         while ( index-- )
         {
            ...
            // Thankfully, because of the above, the new configuration would have a UUID
            // so we can use the find method as we did before.
            // log.debug( "   UUID searching for is" + existingLinkedAccessorysConfig.UUID );

            // See if the linked accessory config exists in the accessories linked types array.
            ...
            // If the linkedAccessory does not exist in the new config, then we need to remove it.
            if ( existingLinkedAccessorysArrayIndex == -1 )
            {
               // log.debug( "   Removing existing config name index: " + index + " is: " + existingLinkedAccessorysConfig.name);

               // Remove the linked Accessory as it was not found from the existing linkedAccessoriesConfig array.
         ...
         // There are no linked accessories at all in the new config, they all need to be removed.
   ...
   {
      // It is possable that there are now no added accessories where there
      // was some before. Handle where they do exist.
      // log.debug( "   new config has added Accessories" );
      if ( Array.isArray( accessory.accessories ) )
      {
         // Iterate over each of the existing configurations of the added accessories, backwards
         // so it is easy to remove them.
         let index = existingAccessory.accessoriesConfig.length;
         while ( index-- )
         {
            // First get the existing Added Accessories config and make sure it has a name.
            // existing Accessory has => accessoriesConfig
            // new Accessory has => accessories
            ...
            // Thankfully, because of the above, the new configuration would have a UUID
            // so we can use find method as we did before.
            // log.debug( "   UUID searching for is" + existingAccessorysConfig.UUID );

            // See if the Added Accessory config exists in the new accessories Config array.
            ...
            // If the addedAccessory does not exist in the new config, then we need to remove it.
            if ( existingAccessorysArrayIndex == -1 )
            {
               // log.debug( "   Removing existing config name index: " + index + " is: " + existingAccessorysConfig.name);
               // Remove the added Accessory as it was not found from the existing accessories array.
               existingAccessory.accessoriesConfig.splice( index, 1 );

            } else {
               // The existing Added accessory already exists in the new config, so nothing
               // needs to be done as it would have been modded up above.
               // log.debug(`   Same addededAccessory ${ existingAccessorysConfig.name }\n`);
            }
         }
      } else {
         // There are no added accessories at all in the new config, they all need to be removed.
